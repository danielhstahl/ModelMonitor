...
def create_train_dataset(spark):
    ...
    return df.withColumn(
        "label", udfConvertToLabel(df["v1"], df["v2"], df["v3"], df["v4"], df["v5"])
    )
...
class TestEndToEndIntegration:
    ...
    def test_end_to_end_integration(self):
        ...
        assembleV = VectorAssembler(
            inputCols=["v1_idx", "v2", "v3", "v4", "v5_idx"], outputCol="features"
        )
        model = RandomForestClassifier(
            labelCol="label", featuresCol="features", numTrees=10
        )
        p = Pipeline(stages=[encodeV1, encodeV5, assembleV, model]).fit(
            self.train_dataset
        )
        ssxi = ssx.StateSpaceXploration(42)
