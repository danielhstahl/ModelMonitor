...
def saveDistribution(
    dataframe, columnNameAndTypeArray: List[Tuple[str, str]], path: str
) -> bool:
...
def getNewDistributionsAndCompare(
    newDataSet, columnNameAndTypeArray: List[Tuple[str, str]], path: str
) -> dict:
    cdr = SimpleConceptDrift()
...
class SimpleConceptDrift(object):
    ...
    def saveDistribution(self, dataframe, columnNameAndTypeArray, path) -> bool:
        ...
        return self.ConceptDrift.saveDistribution(
            self.ConceptDrift.getDistributions(dataframe._jdf, _jColNameTypeArray), path
        )

    def getNewDistributionsAndCompare(
        self, newDataframe, columnNameAndTypeArray, path
    ) -> dict:
        results = self.ConceptDrift.getNewDistributionsAndCompare(
            newDataframe._jdf, self.ConceptDrift.loadDistribution(path)
        )
